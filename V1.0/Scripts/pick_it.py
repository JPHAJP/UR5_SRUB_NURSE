    ...
    while True:
        try:
            if len(opciones_menu)>0:
                resp = random.choice(list(opciones_menu.keys()))
                time.sleep(1)
                print(f"Seleccionando la clase {opciones_menu[resp]}")
...
def main(control):
    ...
    model = YOLO(r'V1.0/Models/V6_best.pt')
    ...
    try:
        while True:
            ...
            while True:
                ...
                # Salir del bucle despues de 5 segundos con un contador sin sleep verficando la hora
